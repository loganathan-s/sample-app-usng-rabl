...
Api is available to clients to retireve the blog categories, blog entries and can able to
comment an blog entry vi REST api in JSON format. And this application might contain
...
3. API to get all Blog Entries(Admin):
      http://rabl-app.herokuapp.com/blog_entries.json?token=auth_token
      
4. API to get all Blog Categories(Admin):
       http://rabl-app.herokuapp.com/blog_entries.json?token= auth_token

5. API to get all Blog Categories:
       http://rabl-app.herokuapp.com/blog_categories.json

6. API to comment on Blog Entry.
	URL: http://rabl-app.herokuapp.com//comments.json
	Request Body:
	   {
	    "token": "authtoken",
	    "comment": {
...
1. User
       Email: demo1@test.com, demo2@test.com
       Password: 123456
