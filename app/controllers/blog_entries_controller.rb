class BlogEntriesController < ApplicationController
  ...
  def index
    @blog_entries = current_user ? current_user.blog_entries.paginate(:page => params[:page]) : BlogEntry.paginate(:page => params[:page])
  ...
  def create
    @blog_entry = current_user.blog_entries.new(params[:blog_entry])
    store_location
    respond_to do |format|
      if @blog_entry.save
  ...
  private
   def admin?
    @blog_entry = BlogEntry.find(params[:id])
    (current_user ? current_user.blog_entrries.include?(@blog_entry) : false)
    respond_to do |format|
